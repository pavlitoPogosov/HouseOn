import { getPhoneWithoutMask } from './stringUtils';
...
export const isValidMobilePhone = (phone: string | undefined) => {
  const phoneWithoutMask = getPhoneWithoutMask(phone || '');
  ...
  for (const key in phones) {
    if (phones.hasOwnProperty(key)) {
      ...
      if (phone.test(phoneWithoutMask)) {
        return true;
      }
    }
  }
